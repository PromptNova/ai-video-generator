                ...
                POST /apply-hook      → apply selected hook to existing session
                    POST /translate       → translate script to target language
                        GET  /sessions/{id}   → get session data
                            ...
                            Requirements:
                                pip install fastapi uvicorn python-multipart aiofiles openai pydub gtts anthropic httpx
...
class SessionStore:
      ...
      def create(self, session_id: str, data: dict):
                ...
                path.write_text(json.dumps(
                    {k: v for k, v in data.items() if k != "file_path"},
                    default=str
                ))
...
def normalize_mix(mix_raw) -> float:
      """
          Accept mix as 0-100 (from frontend slider) or 0.0-1.0 (direct float).
              Always returns a 0.0-1.0 float for ffmpeg.
                  """
    try:
              v = float(mix_raw)
except Exception:
...
async def generate_tts(
      ...
      if os.getenv('ELEVENLABS_API_KEY'):
                try:
                              ...
                              similarity = 0.75
                              if voice == "energetic":
                                                stability = 0.35; similarity = 0.8
      elif voice == "calm":
                stability = 0.75; similarity = 0.7
elif voice == "deep":
                stability = 0.8; similarity = 0.85
...
async def process_video(video_path, audio_path, output_path, offset=0.0, mix=0.5, fade_in=0, fade_out=0, **kwargs):
      try:
                delay_ms = int(offset * 1000)
                vo_vol = round(mix, 2)
                orig_vol = round(1.0 - mix, 2)
                filter_complex = f"[1:a]adelay={delay_ms}|{delay_ms},volume={vo_vol}[vo];[0:a]volume={orig_vol}[orig];[orig][vo]amix=inputs=2:duration=longest[aout]"
                cmd = [
                    'ffmpeg', '-y',
                    '-i', str(video_path),
                    '-i', str(audio_path),
                ...
                if proc.returncode == 0 and output_path.exists():
                              return output_path
        raise Exception(f"ffmpeg error: {stderr.decode()[-300:]}")
except Exception as e:
        log.warning(f"Video merge failed: {e} — returning audio only")
        audio_only = output_path.with_name("output.mp3")
        shutil.copy(audio_path, audio_only)
        return audio_only
...
async def list_voices():
      return {
                "voices": [
                              {"id": "us_clear", "name": "US Clear", "flag": "\U0001f1fa\U0001f1f8"},
                              {"id": "uk_clear", "name": "UK Clear", "flag": "\U0001f1ec\U0001f1e7"},
                              {"id": "au_clear", "name": "AU Clear", "flag": "\U0001f1e6\U0001f1fa"},
                              {"id": "in_clear", "name": "IN Clear", "flag": "\U0001f1ee\U0001f1f3"},
                              {"id": "energetic", "name": "Energetic", "flag": "\u26a1"},
                              {"id": "calm", "name": "Calm", "flag": "\U0001f30a"},
                              {"id": "deep", "name": "Deep", "flag": "\U0001f50a"},
                              {"id": "warm", "name": "Warm", "flag": "\u2600\ufe0f"},
                ],
                "tts_engine": (
                              "elevenlabs" if ELEVENLABS_KEY else "openai" if OPENAI_API_KEY else "gtts"
                ),
      }
